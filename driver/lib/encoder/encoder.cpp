...
int get_encoder_direction(encoder_t encoder_local)
{
	...
	unsigned int current_value = 2*digitalRead(encoder_local.pin_outputA)+digitalRead(encoder_local.pin_outputB);//calculo el valor actual con el mismo criterio que antes, uso binario grey
	int result = 0;
